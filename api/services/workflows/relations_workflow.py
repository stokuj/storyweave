...
from api.kafka.producer import TOPIC_RELATIONS_RESULTS, send_json
...
def process_chapter_relations(
    ...
    if characters and len(characters) >= 2:
        logger.info(
            "Using provided characters %s for chapter %s", characters, chapter_id
        )
        top_chars = characters
    else:
        logger.info(
            "No characters provided for chapter %s, running NER locally", chapter_id
        )
        ner_result = extract_entities(
            TextContentRequest(content=content), DEFAULT_NER_MODEL
        )
        all_chars = list(ner_result.get("characters", {}).keys())

        if len(all_chars) < 2:
            logger.info(
                "Not enough characters in chapter %s to find relations", chapter_id
            )
            return {"relations": []}
...
async def process_book_relations_async(pairs: list[dict], book_id: int | str) -> dict:
    ...
    send_json(TOPIC_RELATIONS_RESULTS, str(book_id), result)
    return result
